import logging
from nifty.storage.database import get_active_levels, deactivate_level, log_trigger
import nifty.storage.portfolio as portfolio
from nifty.core.trade_logic import calculate_trade_params
...
class TriggerChecker:
    def __init__(self, alert_callback):
        ...
        self.triggered_ids = set() # Safety lock to prevent double alerts
    ...
    async def check_ticks(self, current_price: float):
        ...
        for level in active_levels:
            ...
            if level_id in self.triggered_ids:
                continue # Already alerted in this session
                
            target_level = float(level["price"])

            # Touch logic: No candle close needed — pure wick/LTP touch
            touched = (
    ...
    async def trigger_signal(self, level: dict, current_price: float):
        ...
        # 4. Fire Telegram alert
        await self.alert_callback(trade)

        logger.info(f"✅ NIFTY Signal Triggered: {level_type} at {level_price} | LTP: {current_price}")
